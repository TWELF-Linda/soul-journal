...
// ── Preset types (matching your handbook) ─────────────────────────
const PRESET_TYPES = {
  anime:   { label:'Anime',        icon:'⚔️',  color:'#534AB7', bg:'#EEEDFE', text:'#3C3489',
    fields:[{id:'creator',label:'原作 / 製作'},{id:'episodes',label:'集數'},{id:'platform',label:'觀看平台'}], quote:false },
  book:    { label:'Book',         icon:'📚',  color:'#0F6E56', bg:'#E1F5EE', text:'#085041',
    fields:[{id:'author',label:'作者'},{id:'pages',label:'頁數'},{id:'publisher',label:'出版社'}], quote:true },
  drama:   { label:'Drama',        icon:'📺',  color:'#185FA5', bg:'#E6F1FB', text:'#0C447C',
    fields:[{id:'cast',label:'主演'},{id:'episodes',label:'集數'},{id:'country',label:'國家 / 語言'},{id:'platform',label:'觀看平台'}], quote:false },
  musical: { label:'Musical',      icon:'🎭',  color:'#993556', bg:'#FBEAF0', text:'#72243E',
    fields:[{id:'cast',label:'演員'},{id:'fav_song',label:'最喜歡的歌'},{id:'theater',label:'劇團 / 導演'},{id:'duration',label:'時長'},{id:'country',label:'國家 / 語言'},{id:'ticket_price',label:'票價'}], quote:false },
  movie:   { label:'Movie',        icon:'🎬',  color:'#993C1D', bg:'#FAECE7', text:'#712B13',
    fields:[{id:'director',label:'導演'},{id:'cast',label:'主演'},{id:'country',label:'國家'},{id:'duration',label:'時長'}], quote:true },
  variety: { label:'Variety Show', icon:'🎉',  color:'#854F0B', bg:'#FAEEDA', text:'#633806',
    fields:[{id:'host',label:'主持 / 卡司'},{id:'episodes',label:'集數 / 季數'},{id:'platform',label:'平台 / 頻道'}], quote:false },
}
  ...
  return (
    <div
      ...
      style={{
        ...
        transition:'border-color .15s', marginBottom:8,
      }}>

      {/* Poster */}
      <div style={{
        width:66, flexShrink:0, background:'var(--warm)',
  ...
  return (
    <div style={{
      ...
      <div style={{ display:'flex', borderBottom:'0.5px solid var(--border)' }}>
        {/* Big poster */}
        <div style={{
          width:92, flexShrink:0, minHeight:124, background:'var(--card)',
          display:'flex', alignItems:'center', justifyContent:'center',
        ...
        <div style={{ flex:1, padding:12 }}>
          ...
          <TypePill label={t.label} color={t.color} bg={t.bg} text={t.text} />
          {record.date && (
            <div style={{ fontSize:12, color:'var(--text-light)', marginTop:6 }}>📅 {record.date}</div>
          )}
          {record.rating > 0 && (
            <div style={{ marginTop:4 }}><Stars n={record.rating} size={14} /></div>
          )}
        </div>
      </div>
      ...
      <div style={{ padding:12 }}>
        {(t.fields || []).map(f =>
          record.fields?.[f.id] ? (
            <div key={f.id} style={{ display:'flex', gap:6, marginBottom:5, fontSize:13 }}>
              <span style={{ color:'var(--text-light)', flexShrink:0, minWidth:68 }}>{f.label}</span>
        ...
        {record.quote && (
          <div style={{
            borderLeft:'2px solid var(--border)', padding:'6px 10px', marginTop:8,
        ...
        <div style={{ display:'flex', gap:8, marginTop:12, justifyContent:'flex-end' }}>
          ...
          <button onClick={onDelete} style={{
            padding:'6px 14px', borderRadius:20, border:'1.5px solid #F4C2C2',
            background:'none', color:'#C45A5A', fontSize:12, cursor:'pointer',
            display:'flex', alignItems:'center', gap:4,
          }}>🗑 刪除</button>
        </div>
      </div>
...
// ── Field Editor (reusable inside forms) ──────────────────────────
function FieldEditor({ fields, onChange }) {
  const add = () => onChange([...fields, { id:'f_'+Date.now(), label:'新欄位' }])
  const remove = i => onChange(fields.filter((_, j) => j !== i))
  const rename = (i, val) => {
    const next = [...fields]; next[i] = { ...next[i], label:val }; onChange(next)
  }
  return (
    <div>
      {fields.map((f, i) => (
        <div key={f.id} style={{ display:'flex', alignItems:'center', gap:6, marginBottom:6 }}>
          ...
          <button onClick={() => remove(i)} style={{
            ...
            color:'var(--text-light)', fontSize:18, padding:0, lineHeight:1,
          }}>×</button>
        </div>
      ))}
...
function NewTypeModal({ onSave, onClose }) {
  ...
  const [fields, setFields] = useState([{ id:'f1', label:'備註' }])
  ...
  return (
    <div style={{
      position:'fixed', inset:0, background:'rgba(74,55,40,0.3)', zIndex:200,
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ marginBottom:10 }}>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="例如：Concert、Podcast..."
            style={{ width:'100%', padding:'9px 12px', border:'1.5px solid var(--border)', borderRadius:10,
              fontSize:14, color:'var(--text)', background:'var(--card)', outline:'none' }} />
        </div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {ICON_OPTIONS.map(ic => (
              <button key={ic} onClick={() => setIcon(ic)} style={{
                ...
                border: ic === icon ? '2px solid var(--accent)' : '1.5px solid var(--border)',
                background: ic === icon ? 'var(--warm)' : 'var(--card)',
              }}>{ic}</button>
            ))}
          </div>
        </div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
            {COLOR_OPTIONS.map((c, i) => (
              <button key={i} onClick={() => setColorIdx(i)} style={{
                ...
                border: colorIdx === i ? '3px solid var(--text)' : '2px solid transparent',
              }} />
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:12, fontWeight:700, color:'var(--text-light)', display:'block', marginBottom:6 }}>預設欄位</label>
          <FieldEditor fields={fields} onChange={setFields} />
        </div>

        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
          <button onClick={onClose} style={{
            padding:10, borderRadius:20, border:'1.5px solid var(--border)',
            background:'var(--warm)', color:'var(--text)', fontSize:13, fontWeight:600, cursor:'pointer',
          }}>取消</button>
          <button onClick={save} style={{
            padding:10, borderRadius:20, border:'none',
            background:'var(--accent3)', color:'#fff', fontSize:13, fontWeight:700, cursor:'pointer',
          }}>✓ 建立</button>
        </div>
      </div>
    </div>
...
function RecordFormModal({ typesCfg, editRecord, nextId, settings, onSave, onClose }) {
  ...
  const [localFields, setLocalFields] = useState(null) // overrides type fields if edited
  ...
  function handleSave() {
    ...
    // If fields were edited, persist them back to the type config
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display:'flex', gap:12, marginBottom:12 }}>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', gap:8 }}>
            <div>
              <label style={{ fontSize:11, fontWeight:700, color:'var(--text-light)', display:'block', marginBottom:3 }}>標題 *</label>
              <input value={title} onChange={e => setTitle(e.target.value)} placeholder="作品名稱" style={inputStyle} />
            </div>
            <div>
        ...
        {/* Dynamic fields — rendered in pairs */}
        {(() => {
          ...
          for (let i = 0; i < fs.length; i += 2) {
            ...
            rows.push(
              <div key={i} style={{ display:'grid', gridTemplateColumns:b?'1fr 1fr':'1fr', gap:8, marginBottom:8 }}>
                <div>
                  <label style={{ fontSize:11, fontWeight:700, color:'var(--text-light)', display:'block', marginBottom:3 }}>{a.label}</label>
                  <input value={fieldVals[a.id]||''} onChange={e => setFv(a.id, e.target.value)} style={inputStyle} />
                </div>
                {b && <div>
                  <label style={{ fontSize:11, fontWeight:700, color:'var(--text-light)', display:'block', marginBottom:3 }}>{b.label}</label>
                  <input value={fieldVals[b.id]||''} onChange={e => setFv(b.id, e.target.value)} style={inputStyle} />
                </div>}
              </div>
            )
          }
        ...
        {/* Quote (only for types that have it) */}
        {t.quote && (
        ...
        <button onClick={() => setShowFieldEditor(v => !v)} style={{
          ...
          ⚙️ {showFieldEditor ? '收起欄位編輯' : '編輯此類型的欄位'}
        </button>
        {showFieldEditor && (
...
export default function MediaPage({ appData }) {
  ...
  // Custom types are persisted in settings
  ...
  function persistTypes(next) {
    setTypesCfg(next)
    // Save custom (non-preset) types to settings
    const custom = {}
  ...
  function handleSave(rec) {
    ...
    // Persist any field edits back to the type config
    if (rec.type && typesCfg[rec.type]) {
      persistTypes({ ...typesCfg })
    }
  ...
  return (
    <div className="fade-in">
        ...
        {Object.entries(typesCfg).map(([k, ty]) => {
          ...
          return (
            <button key={k} style={filterBtnStyle(filter===k)} onClick={() => { setFilter(k); setActiveDetail(null) }}>
              ...
              {!PRESET_TYPES[k] && (
                <span
                  onClick={e => { e.stopPropagation(); deleteType(k) }}
                  style={{ marginLeft:1, fontSize:11, opacity:.5, cursor:'pointer' }}
                  title="刪除此類型">×</span>
              )}
            </button>
          )
      ...
      {/* Add button */}
      <button
        onClick={() => { setEditRecord(null); setModal('add') }}
        style={{
